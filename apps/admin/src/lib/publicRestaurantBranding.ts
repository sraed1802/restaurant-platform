...
const DEFAULTS: PublicRestaurantBranding = {
  ...
  restaurant_tagline_en: 'Operations',
  restaurant_tagline_ar: 'العمليات',
  logo_url: null,
...
export async function fetchPublicRestaurantBranding(): Promise<PublicRestaurantBranding> {
  const { data, error } = await supabase
    ...
    .select(
      'restaurant_name_en, restaurant_name_ar, restaurant_tagline_en, restaurant_tagline_ar, logo_url, loading_logo_url, admin_shell_logo_url, welcome_logo_url, native_loading_text_en, native_loading_text_ar, primary_color',
    )
    .maybeSingle()
